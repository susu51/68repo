...
class GPSAddressTester:
    ...
    def debug_admin_user_info(self):
        ...
        try:
            response = self.admin_session.get(f"{BACKEND_URL}/me")
            if response.status_code == 200:
                data = response.json()
    ...
    def test_check_business_gps_coverage(self):
        ...
        try:
            # Use admin session cookies from login
            response = self.admin_session.get(f"{BACKEND_URL}/admin/utils/check-business-gps")
            
            if response.status_code == 200:
                data = response.json()
                
                # Check if response has summary structure
                if "summary" in data:
                    ...
                    if not missing_fields:
                        ...
                        with_gps = summary["with_gps"]
                        without_gps = summary["without_gps"]
                        coverage = summary["gps_coverage_percentage"]
                        
                        # Verify math consistency
                        math_correct = (with_gps + without_gps == total) and (coverage == round((with_gps / total * 100) if total > 0 else 0, 2))
                        
                        if math_correct:
                            details = f"Total: {total}, With GPS: {with_gps}, Without GPS: {without_gps}, Coverage: {coverage}%"
                        ...
                        self.log_test("Check Business GPS Coverage", False, "", f"Missing required fields in summary: {missing_fields}")
                        return None
                else:
                    # Try old format
                    required_fields = ["total_businesses", "with_gps", "without_gps", "gps_coverage_percentage"]
                    ...
                    if not missing_fields:
                        ...
                        details = f"Total: {total}, With GPS: {with_gps}, Without GPS: {without_gps}, Coverage: {coverage}%"
                        self.log_test("Check Business GPS Coverage", True, details)
                        return data
                    else:
    ...
    def test_update_business_gps_batch(self):
        ...
        try:
            # Use admin session cookies from login
            response = self.admin_session.post(f"{BACKEND_URL}/admin/utils/update-business-gps")
            ...
            if response.status_code == 200:
                ...
                if "summary" in data:
                    ...
                    if not missing_fields:
                        processed = summary["total_businesses_processed"]
                        ...
                        self.log_test("Update Business GPS Batch", False, "", f"Missing required fields in summary: {missing_fields}")
                        return None
                else:
                    # Try old format
                    required_fields = ["total_businesses_processed", "updated", "failed"]
                    ...
                    if not missing_fields:
                        ...
                        details = f"Processed: {processed}, Updated: {updated}, Failed: {failed}"
                        self.log_test("Update Business GPS Batch", True, details)
                        return data
                    else:
    ...
    def test_verify_gps_coordinates_in_database(self):
        ...
        try:
            # Use admin session cookies from login
            response = self.admin_session.get(f"{BACKEND_URL}/admin/businesses")
    ...
    def test_customer_address_retrieval(self):
        ...
        try:
            # Use customer session cookies from login
            response = self.customer_session.get(f"{BACKEND_URL}/me/addresses")
        ...
        try:
            ...
            for i, test_case in enumerate(test_cases):
                try:
                    ...
                    response = reg_session.post(f"{BACKEND_URL}/register/business", json=registration_data)
