#!/usr/bin/env python3
"""
🎯 E2E BACKEND API TESTING - ORDER FLOW (FIXED)
Comprehensive testing of the complete order flow with seeded E2E test data
Uses cookie-based authentication instead of JWT tokens
"""
...
class E2EOrderFlowTester:
    ...
    def test_authentication_all_roles(self):
        """Test authentication for all 4 roles using cookie-based auth"""
        ...
        for role, credentials in TEST_CREDENTIALS.items():
            try:
                # Create a session for this role
    ...
    def test_businesses_endpoint(self):
        ...
        try:
            # Use any session for public endpoint
            session = list(self.sessions.values())[0] if self.sessions else requests.Session()
            response = session.get(f"{BACKEND_URL}/businesses")
            
            if response.status_code == 200:
                data = response.json()
                businesses = data if isinstance(data, list) else data.get("businesses", [])
                
                # Look for "Niğde Lezzet" business or business-e2e-001
                target_business_found = False
                for business in businesses:
                    business_name = business.get("name", "") or business.get("business_name", "")
                    business_id = business.get("id", "")
                    if ("Niğde Lezzet" in business_name or 
                        "business-e2e-001" == business_id):
                        target_business_found = True
                        break
                
                if target_business_found:
                    self.log_test(
                        "Businesses Endpoint", 
                        True, 
                        f"Found {len(businesses)} businesses including target business"
                    )
                    return True
                else:
                    business_names = [b.get("name", b.get("business_name", "")) for b in businesses]
                    business_ids = [b.get("id", "") for b in businesses]
                    self.log_test(
                        "Businesses Endpoint", 
                        False, 
                        f"Target business not found. Names: {business_names}, IDs: {business_ids}"
                    )
                    return False
            else:
    ...
    def test_business_menu_endpoint(self):
        """Test GET /api/business/business-e2e-001/menu (verify 5 products)"""
        print("📋 Testing Business Menu Endpoint...")
        
        try:
            # Use any session for public endpoint
            session = list(self.sessions.values())[0] if self.sessions else requests.Session()
            response = session.get(f"{BACKEND_URL}/business/business-e2e-001/menu")
    ...
    def test_business_authenticated_menu(self):
        ...
        if "business" not in self.sessions:
            self.log_test("Business Authenticated Menu", False, "Business session not available")
            return False
        
        try:
            response = self.sessions["business"].get(f"{BACKEND_URL}/business/menu")
            ...
            if response.status_code == 200:
                ...
                self.log_test("Business Authenticated Menu", False, "Access forbidden (403) - may need KYC approval", response.text)
                return False
            else:
    ...
    def test_customer_addresses(self):
        ...
        if "customer" not in self.sessions:
            self.log_test("Customer Addresses", False, "Customer session not available")
            return False
        
        try:
            response = self.sessions["customer"].get(f"{BACKEND_URL}/me/addresses")
            ...
            if response.status_code == 200:
                ...
                for address in addresses:
                    ...
                    address_text = str(address.get("address", "")) + str(address.get("city", "")) + str(address.get("il", ""))
                    if "Niğde" in address_text:
                ...
                if len(addresses) > 0:
                    self.log_test(
                        "Customer Addresses", 
                        True, 
                        f"Found {len(addresses)} addresses. Default: {default_address_found}, Niğde: {nigde_address_found}"
                    )
                    return True
                else:
                    self.log_test(
                        "Customer Addresses", 
                        False, 
                        f"No addresses found for customer"
                    )
                    return False
            else:
    ...
    def test_business_incoming_orders(self):
        ...
        if "business" not in self.sessions:
            self.log_test("Business Incoming Orders", False, "Business session not available")
            return False
        
        try:
            response = self.sessions["business"].get(f"{BACKEND_URL}/business/orders/incoming")
            ...
            if response.status_code == 200:
                ...
                self.log_test("Business Incoming Orders", False, "Access forbidden (403) - may need KYC approval", response.text)
                return False
            else:
    ...
    def test_order_status_updates(self):
        ...
        if "business" not in self.sessions or not self.created_order_id:
            self.log_test("Order Status Updates", False, "Business session or order ID not available")
            return False
        
        try:
            # Test 1: created → preparing
            update_payload = {"from": "created", "to": "preparing"}
            response = self.sessions["business"].patch(
    ...
    def run_all_tests(self):
        ...
        print("🎯 E2E BACKEND API TESTING - ORDER FLOW (FIXED)")
        print("=" * 60)
