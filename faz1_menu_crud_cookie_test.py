...
# Configuration
BASE_URL = "https://courier-dashboard-3.preview.emergentagent.com/api"
...
def test_health_and_public_endpoints():
    ...
    try:
        response = requests.get(f"https://courier-dashboard-3.preview.emergentagent.com/menus", headers=HEADERS)
        if response.status_code == 200:
            data = response.json()
